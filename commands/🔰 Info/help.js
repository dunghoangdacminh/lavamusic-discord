...
module.exports = {
  ...
  run: async (client, message, args, cmduser, text, prefix, player, es, ls) => {
	...
	// Thay thế emoji tùy chỉnh trong các button
    ...
    // Thay thế emoji tùy chỉnh trong Select Menu options
    let menuOptions = [{
        label: "Overview",
        value: "Overview",
        emoji: "📋",
...
Made by [**Milrato Development**](https://discord.gg/milrato), edited by Akimiya.`)
    ...
    var embed0 = new MessageEmbed()
      .setTitle(`[\`${client.commands.filter((cmd) => cmd.category === "🔰 Info").size}\`] ℹ️ Information Commands ℹ️`)
      ...
      .addField("🛠️ **Server Related Commands**", ">>> " + client.commands.filter((cmd) => cmd.category === "🔰 Info" && cmd.type === "server").sort((a, b) => a.name.localeCompare(b.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
    ...
    var embed5 = new MessageEmbed()
      ...
      .addField("\u200b", "__**Sub-Categorized Commands:**__")
      .addField("🤖 **Bot Related Commands**", "> " + client.commands.filter((cmd) => cmd.category === "💰 Premium" && cmd.type?.includes("bot")).sort((a, b) => a.name.localeCompare(b.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
      .addField("🎵 **Music Related Commands**", "> " + client.commands.filter((cmd) => cmd.category === "💰 Premium" && cmd.type?.includes("music")).sort((a, b) => a.name.localeCompare(b.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
    embeds.push(embed5)
    ...
    var embed9 = new MessageEmbed()
      ...
      .addField("\u200b", "__**Sub-Categorized Commands:**__")
      .addField("🛠️ **Information & Manage**", "> " + client.commands.filter((cmd) => cmd.category === "👑 Owner" && cmd.type?.includes("info")).sort((a, b) => a.name.localeCompare(b.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
